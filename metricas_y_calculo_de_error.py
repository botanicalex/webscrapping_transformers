...
def calcular_metricas(ruta_excel: str, salida: str) -> Dict[str, Any]:
    ...
    graficos_dir = os.path.join(salida, cfg.DIRECTORIO_GRAFICOS_METRICAS, f"corrida_{timestamp}")
    excel_salida = os.path.join(salida, f"{cfg.PREFIJO_RESULTADO_COMPARACION}_{timestamp}.xlsx")
    ...
    if not experimentos:
        raise ValueError(
            "No se encontraron columnas de experimento (ejemplo: experimento_1, experimento_2)"
        )
    ...
    for exp_col in experimentos:
        ...
        sns.scatterplot(
            data=temp, x="radar_oficial_promedio", y="radar_experimento", s=90, color="steelblue"
        )
...
if __name__ == "__main__":
    calcular_metricas(cfg.ARCHIVO_COMPARACION_EXCEL, ".")
