...
class CalculadorRadar:
    ...
    def _calcular_indicadores_transformers(
        ...
        cols = [c for c in self.COLUMNAS_BINARIAS if c in df_riesgo.columns]
        radar_raw = df_riesgo[cols].mean(axis=1)

        df_sub = pd.DataFrame(index=df_riesgo.index)
        df_sub['n_articulos'] = n_articulos.reindex(df_sub.index)
        for b, variables in self.BLOQUES_PCA.items():
            v = [x for x in variables if x in df_riesgo.columns]
            df_sub[b] = df_riesgo[v].mean(axis=1) * 100.0 if v else 0.0
    ...
    def _calibrar_escala(s: pd.Series,
        ...
        return pd.Series([media_obj] * len(s), index=s.index)
...
def _seleccionar_top_experimentos_desde_metricas(
    ...
    for col in ["score_ranking", "accuracy", "f1_macro", "cohen_kappa"]:
    ...
    if not criterio:
        ...
        elif "accuracy" in trabajo.columns:
            criterio = "accuracy"
        elif "f1_macro" in trabajo.columns:
            criterio = "f1_macro"
        else:
            raise ValueError("No hay columnas de ranking disponibles para seleccionar top")

    # Para clasificación: mayor accuracy/f1/kappa es mejor (descendente)
    descendentes = {"accuracy", "f1_macro", "cohen_kappa"}
    ...
    for col in ["score_ranking", "accuracy", "f1_macro", "cohen_kappa"]:
        if col in trabajo.columns and col not in columnas_orden:
            ...
            ascendente.append(col not in descendentes)
...
def ejecutar_experimentos_radar(
    ...
    for i in range(numero_iteraciones):
        ...
        if operacion == CalculadorRadar.OPERACION_BLOQUES:
            pesos_utilizados = {"metodo": "promedio_simple_36_indicadores", "calibracion": "zscore_media31.4_std7.6"}
