"""
Corre el pipeline NLI (26 indicadores, pre-filtro social) sobre el corpus
combinado de 32 departamentos y genera UNA tabla de indicadores por articulo
por cada departamento (mismo formato que el Excel de Antioquia).
...
def escribir_tablas(df_proc: pd.DataFrame) -> None:
    ...
    for depto, grupo in df_proc.groupby("departamento"):
        ...
        n_rel = int((grupo["score_social"] >= 0.65).sum()) if "score_social" in grupo.columns else -1
        resumen.append((str(depto), len(df_tabla), n_rel, os.path.basename(ruta)))

    print(f"\n{'Departamento':<26} {'Articulos':>10} {'Relevantes':>11}  Archivo")
    for depto, n, n_rel, arch in sorted(resumen):
        print(f"{depto:<26} {n:>10} {n_rel:>11}  {arch}")
